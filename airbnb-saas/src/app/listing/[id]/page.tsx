...
export default async function ListingDetail(
  ...
  // Structured content (from our generator) lives here if present
  ...
  return (
    <main className="mx-auto max-w-4xl p-6 space-y-6">
      ...
      <header>
        ...
        {(l as any).address && <p className="opacity-70">{(l as any).address}</p>}
        <p className="text-[13px] opacity-50">ID: {(l as any).id}</p>
      </header>

      <div className="grid gap-8 md:grid-cols-2">
        {/* Left: basic facts + raw description (if any) */}
        <section className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-x-8 gap-y-2">
            {(l as any).bedrooms > 0 && (
              <div>Bedrooms: <span className="opacity-70">{(l as any).bedrooms}</span></div>
            )}
            {(l as any).bathrooms > 0 && (
              <div>Bathrooms: <span className="opacity-70">{(l as any).bathrooms}</span></div>
            )}
            {(l as any).squareFeet > 0 && (
              <div>Square feet: <span className="opacity-70">{(l as any).squareFeet}</span></div>
            )}
            {(l as any).tone && (
              <div>Tone: <span className="opacity-70">{(l as any).tone}</span></div>
            )}
          </div>
            ...
            <div className="space-y-3">
              ...
              {res.sections?.space && (
                <div>
                  ...
                  <p className="opacity-90">{res.sections.space}</p>
                </div>
              )}
              {res.sections?.access && (
                <div>
                  <h4 className="font-medium">Guest Access</h4>
                  <p className="opacity-90">{res.sections.access}</p>
                </div>
              )}
              {res.sections?.notes && (
                <div>
                  <h4 className="font-medium">Other Things to Note</h4>
                  <p className="opacity-90">{res.sections.notes}</p>
                </div>
              )}
            </div>
          )}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {res.amenities.map((a, i) => (
                  <span key={i} className="rounded-full border px-2 py-0.5 text-xs opacity-80">{a}</span>
                ))}
              </div>
            </div>
          )}
